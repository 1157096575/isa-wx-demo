...
Component({
  ...
  methods: {
    ...
    _bindChange(e) {
      ...
      switch (scrollType) {
        case 'normal':
          ...
          this.data.tempValue = val.concat()
          this.setData({
            tempValue:this.data.tempValue // isa
          })
          break
        case 'link':
          ...
          if (val.length > 1) {
            val.slice(0, val.length - 1).reduce((t, c, i) => {
              const v = t[c] && t[c].children ? t[c].children : []
          ...
          this.data.tempValue = val.concat()
          this.triggerEvent('exportTempValue',{
    ...
    _bindpickstart(e) {
      let _this = this
      _this.data.scrollEnd = false
      setTimeout(function(){
        _this.data.scrollEnd = true
      },500)
    },
    _openPicker() {
    ...
    _setDefault(inBackData) {
      ...
      switch (scrollType) {
        case 'normal':
          if (listData[0] && listData[0][0] && isPlainObject(listData[0][0])) {
            this.setData({
        ...
        case 'link':
          ...
          if (Array.isArray(defaultPickData) && defaultPickData.length > 0 && defaultPickData.every((v, i) => isPlainObject(v))) {
            ...
            do {
              lastValue.push(0)
              columnsData.push(this._getColumnData(listData))
              listData = listData[0] && listData[0].children ? listData[0].children : []
            } while (listData)
